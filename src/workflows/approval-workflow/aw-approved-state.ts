...
class AWApprovedState implements IAWState{
    workflowId: string;
    responses: IAWStateResponse[];
    workflowStateName: string;
    constructor(workflowId){
        this.workflowId = workflowId;
        this.workflowStateName = stateConstants.approved;
        this.responses = [
            {
    ...
    beforeStateChange(response: string): boolean {
        if(response === stateConstants.pending){
            //Implemention before approved to pending state change flow
        } else if(response === stateConstants.rejected){
            //Implemention before approved to rejected state change flow
        }
        return true;
    }
    afterStateChange(response: string): boolean {
        if(response === stateConstants.pending){
            //Implemention after approved to pending state change flow
        } else if(response===stateConstants.rejected){
            //Implemention after approved to rejected state change flow
        }
        return true
    }
}

export default AWApprovedState;
